...
Table of Contents
   ...
   12. Change History  . . . . . . . . . . . . . . . . . . . . . . .  10
     12.1.  Version draft-fujiwara-dnsop-nsec-aggressiveuse-01 . . .  12
     12.2.  Version draft-fujiwara-dnsop-nsec-aggressiveuse-02 . . .  12
...
5.1.  NSEC
   ...
   If denial of existance can be determined according to the rules set
   out in Section 5.4 of [RFC4035], using NSEC records in the cache,
   then the resolver can immediately return an NXDOMAIN or NODATA
   response.

5.2.  NSEC3

   NSEC3 aggressive negative caching is more difficult than NSEC
   agressive caching.  If the zone is signed with NSEC3, the validating
   ...
   NSEC3.  If it does support aggressive use of NSEC3, it MAY provide a
...
5.4.  Wildcards
   ...
   The last paragraph of [RFC4035] Section 4.5 discusses the aggressive
   use of a cached deduced wildcard (as well as aggressive use of NSEC)
   and recommends that it not be relied upon.
   ...
   independently from aggressive use of NSEC, but, will be more
   effective when both are enabled as the resolver can determine the
...
12.  Change History
   ...
   o  Working group does want the "positive" answers, not just negative
      ones.  This requires readding what used to be Section 7, and some
      fixups / clarifications.
